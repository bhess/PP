...
public interface Circuit {
	 ...
	 * All the rest of the player's input wires will be in the following
	 * indexes.
	 * 
	 * @param name
	...
	/**
	 * Gets the number of the players' input wires, which is the players input
	 * size in bits.
	 * 
	 * @param name
	 *            The name of the player.
	 ...
	 * Each entry will have a name of the variable and the size in bits. The
	 * entries will be arranged from the lsb to the msb of the player input
	 * bits. The first entry (position 0) lsb will be the players' input lsb.
	 ...
	 * The mapping is returned using the Map interface. Each entry contains the
	 * variable name (string) as the key, and the indexes of the matching wires
	 * (int[]) as the value. In the value (int[]) int[0] will be the lsb.
	 * 
	 * @param name
	 *            The name of the player.
	 * @return A Map between the player output variable name (String) and it's
	 *         wires indexes (int[]).
	 */
	 ...
	 * The truth table for a two inputs wires gate range between 0 and 15
	 * (inclusive). This number in binary representation (four bits) is the
	 * gate's truth table. For example: The number 14 is 1110 in its binary form
	 * and there for will represent a NAND gate. The following gates are not
	 * supported: 0, 3, 5, 10, 12,15 (the 0/1 consts, the input wires and there
	 * negation)
	 * <p>
	 * 
	 * @param g
